...
frankenstein_dracula <- gutenberg_download(c(84, 345))
...
What's next after retrieving a book's text? It can be joined with the `gutenberg_metadata` set to have more informative fields than just `gutenberg_id`:
...
frankenstein_dracula %>%
  inner_join(book_titles, by = "gutenberg_id")
```

Having the book as a data frame is especially useful for working with the [tidytext](https://github.com/juliasilge/tidytext) package for text analysis.

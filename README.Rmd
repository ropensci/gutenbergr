...
Search through the metadata to find Jane Austen's _Persuasion_:
```{r}
#| label: find-book
gutenberg_works(title == "Persuasion")
```

_Persuasion_'s `gutenberg_id` is 105. We'll use this ID to download it and also set our cache option to `"persistent"` so that we don't have to re-download it later.
```{r}
...
Multiple works can be downloaded at once. We'll also download Edna St. Vincent Millay's _Renascence and Other Poems_ (`gutenberg_id` 161) and throw in `title` data from the metadata.
```{r}
...
```{r}
#| label: download-multiple-sample
#| echo: false
# For README building, use sample data

...
Search, download, and process public domain texts from the [Project Gutenberg](https://www.gutenberg.org/) collection. Includes

* A function `gutenberg_download()` that downloads one or more works from Project Gutenberg by ID: for instance, `gutenberg_download(84)` downloads the text of [_Frankenstein_](https://www.gutenberg.org/ebooks/84).
...
Suppose we wanted to download Emily Brontë's [_Wuthering Heights_](https://www.gutenberg.org/ebooks/768). We could find the book's ID by filtering:
...
* Project Gutenberg allows harvesting with automated software using [this list of links](https://www.gutenberg.org/robot/harvest?filetypes[]=html).
* We retrieve the book text directly from a default or user-specified mirror using links in the same format. For example, _Frankenstein_ (book 84) is retrieved from `https://aleph.pglaf.org/8/84/84-0.txt`.
* We give priority to retrieving `.zip` files to minimize bandwidth on the mirror. `.txt` files are only retrieved if there is no `.zip`.
* gutenbergr supports both session and persistent caching of downloaded files to prevent redownloading.

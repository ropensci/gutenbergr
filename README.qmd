...
Use the `gutenberg_id` of the book to download it. We'll set a cache option so that we don't have to re-download it later.
```{r}
...
Multiple works can be downloaded at once. We'll add `title` and `author` data from the metadata.
```{r}
...
* [Text Mining with gutenbergr and tidytext](https://docs.ropensci.org/gutenbergr/articles/text-mining.html) - complete analysis workflow with tidytext
